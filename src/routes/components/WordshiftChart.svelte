<script>
    ...
    let { barData, dashboardHeight, dashboardWidth, barChartWidth } = $props();
    
    let margin = ({ top: 80, left: 300, right: 300, bottom: 10 })
    ...
    let xRange = $derived([barChartWidth + margin.left, dashboardWidth - margin.right]);
    ...
    let xTicks = $derived(xScale.ticks(barChartWidth / 80));
